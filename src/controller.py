...
class Controller:
	...
	args:self(class), width(int), height(int)
	return: None
		'''
		pygame.init()
		self.width = width
	...
	def mainLoop(self):
		'''
	sets the state to game or game over
	args: self(class)
	return: None
		'''
		while True:
	...
	def gameLoop(self):
		'''
      creates the game loop and moves the hero, checks for the collisions and redraw the entire screen
      args: self(class)
      return: None
		'''
	...
	def gameOver(self):
		'''
      when the hero dies, ends the game
      args: self(class)
      return: None
		'''
		self.hero.kill()
